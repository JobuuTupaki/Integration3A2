        ...
        <div class="max-w-5xl mx-auto">
          <div class="bg-white dark:bg-white-800 ">
            <div class="flex m-2 p-2">
              <Link href="/transactions" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded">Back</Link>
            </div>

            <!-- component -->
            <div class="grid place-content-center mt-5">
              <form @submit.prevent="storeTransaction" class="bg-white shadow-md m-7 p-5 rounded grid grid-cols-2 gap-4">
                <div>
                  <label for="title" class="block text-sm font-medium text-gray-700">Transaction ID</label>
                  <div class="mt-2">
                    <input type="number" id="title" name="transaction_id" v-model="form.transaction_id" class="block w-full sm:w-96 transition duration-150 ease-in-out" readonly>
                  </div>
                </div>
                <div>
...
const form = useForm({
    transaction_id: generateTransactionID(), // Generate transaction ID
    customer_name: null,
    weight: null,
    order_date: null,
    description: null,
...
function generateTransactionID() {
    // Generate a unique transaction ID as a number
    return Math.floor(Math.random() * 1000000); // Adjust the range according to your needs
}
